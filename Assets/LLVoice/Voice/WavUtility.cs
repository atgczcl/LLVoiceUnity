...
public class AudioConverter
{
    ...
    {
        public const int HeaderSize = 44; // WAV文件头大小
        public byte[] riffChunkId; // RIFF Chunk ID
        public int riffChunkSize; // RIFF Chunk Size
        public byte[] riffFormat; // RIFF Format
        public byte[] formatChunkId; // Format Chunk ID
        public int formatChunkSize; // Format Chunk Size
        public short audioFormat; // Audio Format
        public short numChannels; // Number of Channels
        public int sampleRate; // Sample Rate
        public int byteRate; // Byte Rate
        public short blockAlign; // Block Align
        public short bitsPerSample; // Bits Per Sample
        public byte[] dataChunkId; // Data Chunk ID
        public int dataChunkSize; // Data Chunk Size

        public WaveHeader(int sampleRate, int numChannels, int bitsPerSample, int pcmDataLength)
        {
            riffChunkId = Encoding.ASCII.GetBytes("RIFF");
            riffChunkSize = HeaderSize + pcmDataLength;
            riffFormat = Encoding.ASCII.GetBytes("WAVE");
            formatChunkId = Encoding.ASCII.GetBytes("fmt ");
            formatChunkSize = 16;
            audioFormat = 1; // PCM
            ...
            dataChunkId = Encoding.ASCII.GetBytes("data");
            dataChunkSize = pcmDataLength;
        }

        public byte[] ToByteArray()
        {
            byte[] headerBytes = new byte[HeaderSize];
            Buffer.BlockCopy(riffChunkId, 0, headerBytes, 0, 4);
            Buffer.BlockCopy(BitConverter.GetBytes(riffChunkSize), 0, headerBytes, 4, 4);
            Buffer.BlockCopy(riffFormat, 0, headerBytes, 8, 4);
            Buffer.BlockCopy(formatChunkId, 0, headerBytes, 12, 4);
            Buffer.BlockCopy(BitConverter.GetBytes(formatChunkSize), 0, headerBytes, 16, 4);
            Buffer.BlockCopy(BitConverter.GetBytes(audioFormat), 0, headerBytes, 20, 2);
            Buffer.BlockCopy(BitConverter.GetBytes(numChannels), 0, headerBytes, 22, 2);
            Buffer.BlockCopy(BitConverter.GetBytes(sampleRate), 0, headerBytes, 24, 4);
            Buffer.BlockCopy(BitConverter.GetBytes(byteRate), 0, headerBytes, 28, 4);
            Buffer.BlockCopy(BitConverter.GetBytes(blockAlign), 0, headerBytes, 32, 2);
            Buffer.BlockCopy(BitConverter.GetBytes(bitsPerSample), 0, headerBytes, 34, 2);
            Buffer.BlockCopy(dataChunkId, 0, headerBytes, 36, 4);
            Buffer.BlockCopy(BitConverter.GetBytes(dataChunkSize), 0, headerBytes, 40, 4);
            return headerBytes;
        }
    }

    public static void ConvertPcmToWav(byte[] pcmData, int sampleRate, int numChannels, int bitsPerSample, string outputPath)
    {
        var header = new WaveHeader(sampleRate, numChannels, bitsPerSample, pcmData.Length);
    ...
    public static void ConvertPcmToMp3(byte[] pcmData, int sampleRate, int numChannels, int bitsPerSample, string outputPath)
    {
        //using (var inputStream = new MemoryStream(pcmData))
        //{
        //    var waveFormat = new WaveFormat(sampleRate, bitsPerSample, numChannels);
        //    using (var waveProvider = new RawSourceWaveStream(inputStream, waveFormat))
        //    {
        //        using (var outputStream = new FileStream(outputPath, FileMode.Create))
        //        {
        //            using (var lameEncoder = new LameMP3FileWriter(outputStream, waveFormat, 128, LAMEPreset.Standard))
        //            {
        //                waveProvider.CopyTo(lameEncoder);
        //            }
        //        }
        //    }
        //}
